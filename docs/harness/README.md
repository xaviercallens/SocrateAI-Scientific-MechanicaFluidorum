...
mkdir -p .claude/skills/preregister .claude/skills/cost-model .claude/skills/resilient-job \
         .claude/skills/autoresearch-screen .claude/skills/publish-gate .claude/agents
...
and the `claim-auditor` and `manuscript-auditor` subagents become available. Review the files first: each one is an instruction
