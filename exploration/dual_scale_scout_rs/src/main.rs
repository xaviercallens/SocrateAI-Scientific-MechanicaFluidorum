...
impl Grid {
    ...
    fn fft3(&self, data: &mut [C], inverse: bool) {
        ...
        // axis 2 (contiguous): parallel over lines
        data.par_chunks_mut(n).for_each(|line| f.process(line));
...
fn b_fft(s: &State, grid: &Grid) -> Vec<[C; 3]> {
    ...
    let mut v: Vec<Vec<C>> = (0..3).map(|c| { let mut g = grid.to_grid(s, c); grid.fft3(&mut g, true); g }).collect();
    // physical gradients d_j v_i
    let mut dv: Vec<Vec<Vec<C>>> = Vec::new();
    for i in 0..3 {
        let mut row = Vec::new();
        for j in 0..3 {
    ...
    let mut w: Vec<Vec<C>> = (0..3).map(|i| {
        let mut g: Vec<C> = (0..n * n * n).into_par_iter().map(|idx| {
            let mut acc = C::new(0.0, 0.0);
            for j in 0..3 { acc = acc + v[j][idx] * dv[i][j][idx]; }
            acc
        }).collect();
        grid.fft3(&mut g, false);
        g.par_iter_mut().for_each(|x| *x = *x / n3);
        g
    }).collect();
    v.clear();
    // convective_k = -(u . grad u)_k on the ball, then Leray
    let out: Vec<[C; 3]> = s.modes.iter().map(|k| {
        let idx = grid.idx_of(k);
        let conv = [-w[0][idx], -w[1][idx], -w[2][idx]];
        leray(k, conv)
    }).collect();
    w.clear();
    out
...
fn phases_adversarial(m: i64) -> std::collections::HashMap<K, C> {
    ...
    let mut table: Vec<(K, K, K, i64)> = Vec::new();
    for p in ball(m) { for q in ball(m) {
        let r = p.add(&q).neg();
        if !bm.contains(&r) || p == K([0,0,0]) || q == K([0,0,0]) || r == K([0,0,0]) { continue; }
        let (rp, rq, rr) = (rep(&p), rep(&q), rep(&r));
        let w = r.sq() - q.sq();
        let g = w * q.dot(&p.cross(&dirs[&rp])) * q.cross(&dirs[&rq]).dot(&r.cross(&dirs[&rr]));
        if g != 0 { table.push((rp, rq, rr, g)); }
    }}
    let mut classes: Vec<K> = table.iter().flat_map(|(a, b, c, _)| [*a, *b, *c]).collect();
    classes.sort(); classes.dedup();
    let mut signs: std::collections::HashMap<K, i64> = classes.iter().map(|c| (*c, 1)).collect();
    let total = |signs: &std::collections::HashMap<K, i64>| -> i64 {
        table.iter().map(|(a, b, c, g)| signs[a] * signs[b] * signs[c] * g).sum::<i64>().abs()
    };
    let mut best = total(&signs);
    loop {
        let mut improved = false;
        for c in &classes {
            *signs.get_mut(c).unwrap() *= -1;
            let v = total(&signs);
            if v > best { best = v; improved = true; } else { *signs.get_mut(c).unwrap() *= -1; }
        }
        if !improved { break; }
    }
    half_ball(m).into_iter().map(|k| (k, C::new(0.0, *signs.get(&k).unwrap_or(&1) as f64))).collect()
}
...
fn scout(args: &[String]) {
    ...
    let phases = if ic == "adv" { phases_adversarial(m) } else { phases_random(m, seed) };
...
fn main() {
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        _ => eprintln!("usage: calibrate <json> | scout --M m --ic adv|null --nu x --dt x --steps n [--every e] [--lambda l] [--engine fft|direct] [--seed s]"),
    }
}
