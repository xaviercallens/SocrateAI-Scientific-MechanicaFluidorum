...
| **OP-6 scoping — instantiating `B` with the real NSE nonlinearity** | Scoped (not drafted) 2026-08-13, **genuinely unspecified territory, E-1 blocker confirmed by repo-wide search** (PLAN.md §6) | `docs/designs/B_INSTANTIATION_SCOPING.md`. No Fourier/triad convolution formula exists anywhere in this program's sanctioned content prior to this memo (confirmed by search: T0.1/T0.2 count triads with no amplitudes; OP-2's draft self-admits it discards angular structure; the dyadic `NL_n` is never asserted to derive from `(u·∇)u`). The standard Fourier-Galerkin form is presented for scoping only, explicitly flagged as **not yet independently verified against a fixed citable source** (only cross-checked against one current web search result, not this program's usual citation bar). Three decision points (D1 full `ℤ³` reindexing vs. reduced proxy; D2 sequencing vs. OP-2; D3 recommended smallest first deliverable — a Tier B triad-convolution + energy-conservation harness) are laid out, none decided. No Lean file touched; nothing here may be cited as a claim. |
